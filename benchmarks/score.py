...
def main(argv: list[str]) -> int:
    ...
    if not rows:
        print("no results")
        return 1
    table: dict[tuple[str, str], dict] = defaultdict(lambda: {"n": 0, "pass": 0, "fv": 0, "score": 0.0, "breach": 0, "errors": 0})
    for row in rows:
        key = (row["set"], row["arm"])
        cell = table[key]
        cell["n"] += 1
        ...
        row["verdict"] = v
    print(f"{'set':<14}{'arm':<9}{'n':>4}{'pass%':>8}{'falseVer':>10}{'rubric':>8}{'breach':>8}{'err':>5}")
    for (set_name, arm), c in sorted(table.items()):
        ...
        print(f"{set_name:<14}{arm:<9}{c['n']:>4}{pass_pct:>8.1f}{c['fv']:>10}{rubric:>8.2f}{c['breach']:>8}{c['errors']:>5}")

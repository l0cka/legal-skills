# Harvey Workflow Builder: Prepare Victorian Estate Planning Drafts

This guide maps `assemble-vic-estate-documents` to one private Harvey Workflow
...
Create one Workflow agent named **Prepare Victorian Estate Planning Drafts**
for one client in a practitioner-led Victorian matter. Start a separate run for
each member of a couple. Do not combine it with the NSW workflow.
...
Create or select a senior-lawyer-maintained Victorian Vault containing only the
current approved drafting materials. Complete these placeholders in the private
workflow build record:

```text
jurisdiction: VIC
Harvey Vault: <ADD EXACT VICTORIAN VAULT NAME>
Will precedent: <ADD EXACT VICTORIAN WILL PRECEDENT FILE NAME>
Enduring power of attorney precedent: <ADD EXACT VICTORIAN EPOA PRECEDENT FILE NAME>
Medical treatment decision-maker precedent: <ADD EXACT VICTORIAN MTDM PRECEDENT FILE NAME>
Instruction sheet: <ADD EXACT VICTORIAN INSTRUCTION SHEET FILE NAME>
Drafting playbook: <ADD EXACT VICTORIAN DRAFTING PLAYBOOK FILE NAME>
```
...
1. Run one complete Victorian instruction set and confirm each draft uses the
...
5. Supply NSW or two-client instructions and confirm the agent stops or routes
   without drafting.
